		...
		public ViewData[,] GenerateViewData()
		{
			...
			for (int x = 0; x < 10; x++)
			{
				for(int y = 0; y < 10; y++)
				{
					int xOffset = 3 + x * 4;
					int yOffset = y * 2;
					if ((!isComputer && BoardData[x, y] == BoardValue.Ship) || BoardData[x, y] == BoardValue.Sank)
					{
						data[xOffset, yOffset] = (x != 0 && (BoardData[x - 1, y] == BoardValue.Ship || BoardData[x - 1, y] == BoardValue.Sank)) ? ViewData.ShipHorizontalDown : (y != 0 && (BoardData[x, y - 1] == BoardValue.Ship || BoardData[x, y - 1] == BoardValue.Sank)) ? ViewData.ShipVerticalRight : ViewData.ShipUpperLeft;
						data[xOffset + 4, yOffset] = (y != 0 && (BoardData[x, y - 1] == BoardValue.Ship || BoardData[x, y - 1] == BoardValue.Sank)) ? ViewData.ShipVerticalLeft : ViewData.ShipUpperRight;
						data[xOffset, yOffset + 2] = (x != 0 && (BoardData[x - 1, y] == BoardValue.Ship || BoardData[x - 1, y] == BoardValue.Sank)) ? ViewData.ShipHorizontalUp : ViewData.ShipLowerLeft;
